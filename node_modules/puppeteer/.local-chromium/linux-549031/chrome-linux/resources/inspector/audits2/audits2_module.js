...
ReportRenderer.AuditJSON;ReportRenderer.CategoryJSON;ReportRenderer.GroupJSON;ReportRenderer.ReportJSON;;Audits2.Audits2Panel=class extends UI.Panel{constructor(){super('audits2');this.registerRequiredCSS('audits2/lighthouse/report-styles.css');this.registerRequiredCSS('audits2/audits2Panel.css');this._protocolService=new Audits2.ProtocolService();this._renderToolbar();this._auditResultsElement=this.contentElement.createChild('div','audits2-results-container');this._dropTarget=new UI.DropTarget(this.contentElement,[UI.DropTarget.Type.File],Common.UIString('Drop audit file here'),this._handleDrop.bind(this));for(const preset of Audits2.Audits2Panel.Presets)
preset.setting.addChangeListener(this._refreshDialogUI.bind(this));this._showLandingPage();SDK.targetManager.observeModels(SDK.ServiceWorkerManager,this);SDK.targetManager.addEventListener(SDK.TargetManager.Events.InspectedURLChanged,this._refreshDialogUI,this);}
modelAdded(serviceWorkerManager){if(this._manager)
return;this._manager=serviceWorkerManager;this._serviceWorkerListeners=[this._manager.addEventListener(SDK.ServiceWorkerManager.Events.RegistrationUpdated,this._refreshDialogUI,this),this._manager.addEventListener(SDK.ServiceWorkerManager.Events.RegistrationDeleted,this._refreshDialogUI,this),];this._refreshDialogUI();}
modelRemoved(serviceWorkerManager){if(!this._manager||this._manager!==serviceWorkerManager)
return;Common.EventTarget.removeEventListeners(this._serviceWorkerListeners);this._manager=null;this._serviceWorkerListeners=null;this._refreshDialogUI();}
_hasActiveServiceWorker(){if(!this._manager)
return false;const mainTarget=SDK.targetManager.mainTarget();if(!mainTarget)
...
return null;const mainTarget=SDK.targetManager.mainTarget();const inspectedURL=mainTarget&&mainTarget.inspectedURL();if(inspectedURL&&!/^(http|chrome-extension)/.test(inspectedURL)){return Common.UIString('Can only audit HTTP/HTTPS pages and Chrome extensions. '+'Navigate to a different page to start an audit.');}
...
_refreshDialogUI(){if(!this._dialog)
return;const hasActiveServiceWorker=this._hasActiveServiceWorker();const hasAtLeastOneCategory=this._hasAtLeastOneCategory();const unauditablePageMessage=this._unauditablePageMessage();const isDisabled=hasActiveServiceWorker||!hasAtLeastOneCategory||!!unauditablePageMessage;let helpText='';if(hasActiveServiceWorker){helpText=Common.UIString('Multiple tabs are being controlled by the same service worker. '+'Close your other tabs on the same origin to audit this page.');}else if(!hasAtLeastOneCategory){helpText=Common.UIString('At least one category must be selected.');}else if(unauditablePageMessage){helpText=unauditablePageMessage;}
this._dialog.setHelpText(helpText);this._dialog.setStartEnabled(!isDisabled);}
...
_clearAll(){this._reportSelector.clearAll();this._showLandingPage();this._refreshToolbarUI();}
_downloadSelected(){this._reportSelector.downloadSelected();}
_renderToolbar(){const toolbar=new UI.Toolbar('',this.element);this._newButton=new UI.ToolbarButton(Common.UIString('Perform an audit\u2026'),'largeicon-add');toolbar.appendToolbarItem(this._newButton);this._newButton.addEventListener(UI.ToolbarButton.Events.Click,this._showDialog.bind(this));this._downloadButton=new UI.ToolbarButton(Common.UIString('Download report'),'largeicon-download');toolbar.appendToolbarItem(this._downloadButton);this._downloadButton.addEventListener(UI.ToolbarButton.Events.Click,this._downloadSelected.bind(this));toolbar.appendSeparator();this._reportSelector=new Audits2.ReportSelector();toolbar.appendToolbarItem(this._reportSelector.comboBox());this._clearButton=new UI.ToolbarButton(Common.UIString('Clear all'),'largeicon-clear');toolbar.appendToolbarItem(this._clearButton);this._clearButton.addEventListener(UI.ToolbarButton.Events.Click,this._clearAll.bind(this));toolbar.appendSeparator();toolbar.appendText(ls`Emulation: `);for(const runtimeSetting of Audits2.Audits2Panel.RuntimeSettings){const control=new UI.ToolbarSettingComboBox(runtimeSetting.options,runtimeSetting.setting);control.element.title=runtimeSetting.description;toolbar.appendToolbarItem(control);}
this._refreshToolbarUI();}
_showLandingPage(){if(this._reportSelector.hasCurrentSelection())
return;this._auditResultsElement.removeChildren();const landingPage=this._auditResultsElement.createChild('div','vbox audits2-landing-page');const landingCenter=landingPage.createChild('div','vbox audits2-landing-center');landingCenter.createChild('div','audits2-logo');const text=landingCenter.createChild('div','audits2-landing-text');text.createChild('span','audits2-landing-bold-text').textContent=Common.UIString('Audits');text.createChild('span').textContent=Common.UIString(' help you identify and fix common problems that affect'+' your site\'s performance, accessibility, and user experience. ');const link=text.createChild('span','link');link.textContent=Common.UIString('Learn more');link.addEventListener('click',()=>InspectorFrontendHost.openInNewTab('https://developers.google.com/web/tools/lighthouse/'));const newButton=UI.createTextButton(Common.UIString('Perform an audit\u2026'),this._showDialog.bind(this),'',true);landingCenter.appendChild(newButton);this.setDefaultFocusedElement(newButton);this._refreshToolbarUI();}
_showDialog(){this._dialog=new Audits2.Audits2Dialog(result=>this._buildReportUI(result),this._protocolService);this._dialog.render(this._auditResultsElement);this._refreshDialogUI();}
_hideDialog(){if(!this._dialog)
return;this._dialog.hide();delete this._dialog;}
_buildReportUI(lighthouseResult){if(lighthouseResult===null)
return;const optionElement=new Audits2.ReportSelector.Item(lighthouseResult,this._auditResultsElement,this._showLandingPage.bind(this));this._reportSelector.prepend(optionElement);this._hideDialog();this._refreshToolbarUI();}
...
_loadedFromFile(profile){const data=JSON.parse(profile);if(!data['lighthouseVersion'])
...
Audits2.Audits2Panel.Preset;Audits2.Audits2Panel.Presets=[{setting:Common.settings.createSetting('audits2.cat_perf',true),configID:'performance',title:'Performance',description:'How long does this app take to show content and become usable'},{setting:Common.settings.createSetting('audits2.cat_pwa',true),configID:'pwa',title:'Progressive Web App',description:'Does this page meet the standard of a Progressive Web App'},{setting:Common.settings.createSetting('audits2.cat_best_practices',true),configID:'best-practices',title:'Best practices',description:'Does this page follow best practices for modern web development'},{setting:Common.settings.createSetting('audits2.cat_a11y',true),configID:'accessibility',title:'Accessibility',description:'Is this page usable by people with disabilities or impairments'},{setting:Common.settings.createSetting('audits2.cat_seo',true),configID:'seo',title:'SEO',description:'Is this page optimized for search engine results ranking'},];Audits2.Audits2Panel.RuntimeSetting;Audits2.Audits2Panel.RuntimeSettings=[{setting:Common.settings.createSetting('audits2.device_type','mobile'),description:Common.UIString('Apply mobile emulation during auditing'),setFlags:(flags,value)=>{flags.disableDeviceEmulation=value==='desktop';},options:[{label:Common.UIString('Mobile'),value:'mobile'},{label:Common.UIString('Desktop'),value:'desktop'},],},{setting:Common.settings.createSetting('audits2.throttling','default'),description:Common.UIString('Apply network and CPU throttling during performance auditing'),setFlags:(flags,value)=>{flags.disableNetworkThrottling=value==='off';flags.disableCpuThrottling=value==='off';},options:[{label:Common.UIString('3G w/ CPU slowdown'),value:'default'},{label:Common.UIString('No throttling'),value:'off'},],},{setting:Common.settings.createSetting('audits2.storage_reset','on'),description:Common.UIString('Reset storage (localStorage, IndexedDB, etc) to a clean baseline before auditing'),setFlags:(flags,value)=>{flags.disableStorageReset=value==='off';},options:[{label:Common.UIString('Clear storage'),value:'on'},{label:Common.UIString('Preserve storage'),value:'off'},],},];Audits2.ReportSelector=class{constructor(){this._emptyItem=null;this._comboBox=new UI.ToolbarComboBox(this._handleChange.bind(this),'audits2-report');this._comboBox.setMaxWidth(180);this._comboBox.setMinWidth(140);this._itemByOptionElement=new Map();this._setPlaceholderState();}
_setPlaceholderState(){this._comboBox.setEnabled(false);this._emptyItem=createElement('option');this._emptyItem.label=Common.UIString('(no reports)');this._comboBox.selectElement().appendChild(this._emptyItem);this._comboBox.select(this._emptyItem);}
...
prepend(item){if(this._emptyItem){this._emptyItem.remove();delete this._emptyItem;}
const optionEl=item.optionElement();const selectEl=this._comboBox.selectElement();this._itemByOptionElement.set(optionEl,item);selectEl.insertBefore(optionEl,selectEl.firstElementChild);this._comboBox.setEnabled(true);this._comboBox.select(optionEl);item.select();}
clearAll(){for(const elem of this._comboBox.options()){this._itemByOptionElement.get(elem).delete();this._itemByOptionElement.delete(elem);}
this._setPlaceholderState();}
downloadSelected(){const item=this._selectedItem();item.download();}};Audits2.ReportSelector.Item=class{constructor(lighthouseResult,resultsView,showLandingCallback){this._lighthouseResult=lighthouseResult;this._resultsView=resultsView;this._showLandingCallback=showLandingCallback;this._reportContainer=null;const url=new Common.ParsedURL(lighthouseResult.url);const timestamp=lighthouseResult.generatedTime;this._element=createElement('option');this._element.label=`${new Date(timestamp).toLocaleTimeString()} - ${url.domain()}`;}
...
download(){const url=new Common.ParsedURL(this._lighthouseResult.url).domain();const timestamp=this._lighthouseResult.generatedTime;const fileName=`${url}-${new Date(timestamp).toISO8601Compact()}.json`;Workspace.fileManager.save(fileName,JSON.stringify(this._lighthouseResult),true);}
...
this._reportContainer=this._resultsView.createChild('div','lh-vars lh-root lh-devtools');const dom=new DOM((this._resultsView.ownerDocument));const detailsRenderer=new Audits2.DetailsRenderer(dom);const categoryRenderer=new Audits2.CategoryRenderer(dom,detailsRenderer);categoryRenderer.setTraceArtifact(this._lighthouseResult);const renderer=new Audits2.Audits2Panel.ReportRenderer(dom,categoryRenderer);const templatesHTML=Runtime.cachedResources['audits2/lighthouse/templates.html'];const templatesDOM=new DOMParser().parseFromString(templatesHTML,'text/html');if(!templatesDOM)
return;renderer.setTemplateContext(templatesDOM);renderer.renderReport(this._lighthouseResult,this._reportContainer);}};Audits2.CategoryRenderer=class extends CategoryRenderer{constructor(dom,detailsRenderer){super(dom,detailsRenderer);this._defaultPassTrace=null;}
...
return element;const timelineButton=UI.createTextButton(Common.UIString('View Trace'),onViewTraceClick,'view-trace');element.querySelector('.lh-audit-group').prepend(timelineButton);return element;async function onViewTraceClick(){await UI.inspectorView.showPanel('timeline');Timeline.TimelinePanel.instance().loadFromEvents(defaultPassTrace.traceEvents);}}};Audits2.DetailsRenderer=class extends DetailsRenderer{constructor(dom){super(dom);this._onMainFrameNavigatedPromise=null;}
renderNode(item){const element=super.renderNode(item);this._replaceWithDeferredNodeBlock(element,item);return element;}
async _replaceWithDeferredNodeBlock(origElement,detailsItem){const mainTarget=SDK.targetManager.mainTarget();if(!this._onMainFrameNavigatedPromise){const resourceTreeModel=mainTarget.model(SDK.ResourceTreeModel);this._onMainFrameNavigatedPromise=resourceTreeModel.once(SDK.ResourceTreeModel.Events.MainFrameNavigated);}
await this._onMainFrameNavigatedPromise;const domModel=mainTarget.model(SDK.DOMModel);if(!detailsItem.path)
...
return;const element=await Common.Linkifier.linkify(node,({title:detailsItem.snippet}));origElement.title='';origElement.textContent='';origElement.appendChild(element);}};;Audits2.Audits2Dialog=class{constructor(onAuditComplete,protocolService){this._onAuditComplete=onAuditComplete;this._protocolService=protocolService;this._protocolService.registerStatusCallback(msg=>this._updateStatus(Common.UIString(msg)));}
render(dialogRenderElement){this._dialog=new UI.Dialog();this._dialog.setOutsideClickCallback(event=>event.consume(true));const root=UI.createShadowRootWithCoreStyles(this._dialog.contentElement,'audits2/audits2Dialog.css');const auditsViewElement=root.createChild('div','audits2-view vbox');const closeButton=auditsViewElement.createChild('div','dialog-close-button','dt-close-button');closeButton.addEventListener('click',()=>this._cancelAndClose());const uiElement=auditsViewElement.createChild('div','vbox launcher-container');const headerElement=uiElement.createChild('header');this._headerTitleElement=headerElement.createChild('p');this._headerTitleElement.textContent=Common.UIString('Audits to perform');uiElement.appendChild(headerElement);this._auditSelectorForm=uiElement.createChild('form','audits2-form');for(const preset of Audits2.Audits2Panel.Presets){preset.setting.setTitle(preset.title);const checkbox=new UI.ToolbarSettingCheckbox(preset.setting);const row=this._auditSelectorForm.createChild('div','vbox audits2-launcher-row');row.appendChild(checkbox.element);row.createChild('span','audits2-launcher-description dimmed').textContent=preset.description;}
this._statusView=new Audits2.Audits2StatusView();this._statusView.render(uiElement);this._dialogHelpText=uiElement.createChild('div','audits2-dialog-help-text');const buttonsRow=uiElement.createChild('div','audits2-dialog-buttons hbox');this._startButton=UI.createTextButton(Common.UIString('Run audit'),this._start.bind(this),'',true);buttonsRow.appendChild(this._startButton);this._cancelButton=UI.createTextButton(Common.UIString('Cancel'),this._cancel.bind(this));buttonsRow.appendChild(this._cancelButton);auditsViewElement.tabIndex=0;this._dialog.setDefaultFocusedElement(this._startButton);this._dialog.setSizeBehavior(UI.GlassPane.SizeBehavior.SetExactWidthMaxHeight);this._dialog.setMaxContentSize(new UI.Size(500,400));this._dialog.show(dialogRenderElement);}
hide(){if(!this._dialog)
return;this._dialog.hide();delete this._dialog;delete this._statusView;delete this._startButton;delete this._cancelButton;delete this._auditSelectorForm;delete this._headerTitleElement;delete this._emulationEnabledBefore;delete this._emulationOutlineEnabledBefore;}
setStartEnabled(isEnabled){if(this._dialogHelpText)
this._dialogHelpText.classList.toggle('hidden',isEnabled);if(this._startButton)
this._startButton.disabled=!isEnabled;}
setHelpText(text){if(this._dialogHelpText)
this._dialogHelpText.textContent=text;}
_updateStatus(message){if(!this._statusView)
return;this._statusView.updateStatus(message);}
_cancelAndClose(){this._cancel();this.hide();}
async _cancel(){if(this._auditRunning){this._updateStatus(Common.UIString('Cancelling\u2026'));await this._stopAndReattach();if(this._statusView)
this._statusView.reset();}else{this.hide();}}
_getInspectedURL(){const mainTarget=SDK.targetManager.mainTarget();const runtimeModel=mainTarget.model(SDK.RuntimeModel);const executionContext=runtimeModel&&runtimeModel.defaultExecutionContext();let inspectedURL=mainTarget.inspectedURL();if(!executionContext)
return Promise.resolve(inspectedURL);return executionContext.evaluate({expression:'window.location.href',objectGroup:'audits',includeCommandLineAPI:false,silent:false,returnByValue:true,generatePreview:false},false,false).then(result=>{if(!result.exceptionDetails&&result.object){inspectedURL=result.object.value;result.object.release();}
return inspectedURL;});}
_getFlags(){const flags={};for(const runtimeSetting of Audits2.Audits2Panel.RuntimeSettings)
runtimeSetting.setFlags(flags,runtimeSetting.setting.get());return flags;}
async _start(){const flags=this._getFlags();const emulationModel=self.singleton(Emulation.DeviceModeModel);this._emulationEnabledBefore=emulationModel.enabledSetting().get();this._emulationOutlineEnabledBefore=emulationModel.deviceOutlineSetting().get();emulationModel.toolbarControlsEnabledSetting().set(false);if(flags.disableDeviceEmulation){emulationModel.enabledSetting().set(false);emulationModel.deviceOutlineSetting().set(false);emulationModel.emulate(Emulation.DeviceModeModel.Type.None,null,null);}else{emulationModel.enabledSetting().set(true);emulationModel.deviceOutlineSetting().set(true);for(const device of Emulation.EmulatedDevicesList.instance().standard()){if(device.title==='Nexus 5X')
emulationModel.emulate(Emulation.DeviceModeModel.Type.Device,device,device.modes[0],1);}}
this._dialog.setCloseOnEscape(false);const categoryIDs=[];for(const preset of Audits2.Audits2Panel.Presets){if(preset.setting.get())
categoryIDs.push(preset.configID);}
Host.userMetrics.actionTaken(Host.UserMetrics.Action.Audits2Started);try{this._auditURL=await this._getInspectedURL();await this._protocolService.attach();this._auditRunning=true;this._updateButton(this._auditURL);this._updateStatus(Common.UIString('Loading\u2026'));const lighthouseResult=await this._protocolService.startLighthouse(this._auditURL,categoryIDs,flags);if(lighthouseResult&&lighthouseResult.fatal){const error=new Error(lighthouseResult.message);error.stack=lighthouseResult.stack;throw error;}
if(!lighthouseResult)
throw new Error('Auditing failed to produce a result');await this._stopAndReattach();await this._onAuditComplete(lighthouseResult);}catch(err){if(err instanceof Error)
this._statusView.renderBugReport(err,this._auditURL);}}
async _stopAndReattach(){await this._protocolService.detach();const emulationModel=self.singleton(Emulation.DeviceModeModel);emulationModel.enabledSetting().set(this._emulationEnabledBefore);emulationModel.deviceOutlineSetting().set(this._emulationOutlineEnabledBefore);emulationModel.toolbarControlsEnabledSetting().set(true);Emulation.InspectedPagePlaceholder.instance().update(true);Host.userMetrics.actionTaken(Host.UserMetrics.Action.Audits2Finished);const resourceTreeModel=SDK.targetManager.mainTarget().model(SDK.ResourceTreeModel);await resourceTreeModel.navigate(this._auditURL);this._auditRunning=false;this._updateButton(this._auditURL);}
_updateButton(auditURL){if(!this._dialog)
return;this._startButton.classList.toggle('hidden',this._auditRunning);this._startButton.disabled=this._auditRunning;this._statusView.setVisible(this._auditRunning);this._auditSelectorForm.classList.toggle('hidden',this._auditRunning);if(this._auditRunning){const parsedURL=(auditURL||'').asParsedURL();const pageHost=parsedURL&&parsedURL.host;this._headerTitleElement.textContent=pageHost?ls`Auditing ${pageHost}\u2026`:ls`Auditing your web page\u2026`;}else{this._headerTitleElement.textContent=Common.UIString('Audits to perform');}}};;Audits2.Audits2StatusView=class{constructor(){this._statusView=null;this._progressWrapper=null;this._progressBar=null;this._statusText=null;this._textChangedAt=0;this._fastFactsQueued=Audits2.Audits2StatusView.FastFacts.slice();this._currentPhase=null;this._scheduledTextChangeTimeout=null;this._scheduledFastFactTimeout=null;}
render(parentElement){this.reset();this._statusView=parentElement.createChild('div','audits2-status vbox hidden');this._progressWrapper=this._statusView.createChild('div','audits2-progress-wrapper');this._progressBar=this._progressWrapper.createChild('div','audits2-progress-bar');this._statusText=this._statusView.createChild('div','audits2-status-text');this.updateStatus(Common.UIString('Loading...'));}
reset(){this._resetProgressBarClasses();clearTimeout(this._scheduledFastFactTimeout);this._textChangedAt=0;this._fastFactsQueued=Audits2.Audits2StatusView.FastFacts.slice();this._currentPhase=null;this._scheduledTextChangeTimeout=null;this._scheduledFastFactTimeout=null;}
setVisible(isVisible){this._statusView.classList.toggle('hidden',!isVisible);if(!isVisible)
...
return Common.UIString(phase.message);const deviceType=Audits2.Audits2Panel.RuntimeSettings.find(item=>item.setting.name==='audits2.device_type').setting.get();const throttling=Audits2.Audits2Panel.RuntimeSettings.find(item=>item.setting.name==='audits2.throttling').setting.get();const match=Audits2.Audits2StatusView.LoadingMessages.find(item=>{return item.deviceType===deviceType&&item.throttling===throttling;});return match?ls`${match.message}`:ls`Lighthouse is loading your page`;}
_getPhaseForMessage(message){return Audits2.Audits2StatusView.StatusPhases.find(phase=>message.startsWith(phase.statusMessagePrefix));}
...
_updateFastFactIfNecessary(){const now=performance.now();if(now-this._textChangedAt<Audits2.Audits2StatusView.fastFactRotationInterval)
...
clearTimeout(this._scheduledTextChangeTimeout);const msSinceLastChange=performance.now()-this._textChangedAt;const msToTextChange=Audits2.Audits2StatusView.minimumTextVisibilityDuration-msSinceLastChange;this._scheduledTextChangeTimeout=setTimeout(()=>{this._commitTextChange(text);},Math.max(msToTextChange,0));}
renderBugReport(err,auditURL){console.error(err);clearTimeout(this._scheduledFastFactTimeout);clearTimeout(this._scheduledTextChangeTimeout);this._resetProgressBarClasses();this._progressBar.classList.add('errored');this._commitTextChange('');this._statusText.createTextChild(Common.UIString('Ah, sorry! We ran into an error: '));this._statusText.createChild('em').createTextChild(err.message);if(Audits2.Audits2StatusView.KnownBugPatterns.some(pattern=>pattern.test(err.message))){const message=Common.UIString('Try to navigate to the URL in a fresh Chrome profile without any other tabs or '+'extensions open and try again.');this._statusText.createChild('p').createTextChild(message);}else{this._renderBugReportLink(err,auditURL);}}
...
${err.stack}
\`\`\`
    `;const body='&body='+encodeURIComponent(issueBody.trim());const reportErrorEl=UI.XLink.create(baseURI+title+body,Common.UIString('Report this bug'),'audits2-link audits2-report-error');this._statusText.appendChild(reportErrorEl);}};Audits2.Audits2StatusView.KnownBugPatterns=[/PARSING_PROBLEM/,/DOCUMENT_REQUEST/,/READ_FAILED/,/TRACING_ALREADY_STARTED/,/^You must provide a url to the runner/,/^You probably have multiple tabs open/,];Audits2.Audits2StatusView.StatusPhases=[{id:'loading',progressBarClass:'loading',statusMessagePrefix:'Loading page',order:10,},{id:'gathering',progressBarClass:'gathering',message:'Lighthouse is gathering information about the page to compute your score.',statusMessagePrefix:'Retrieving',order:20,},{id:'auditing',progressBarClass:'auditing',message:'Almost there! Lighthouse is now generating your own special pretty report!',statusMessagePrefix:'Evaluating',order:30,}];Audits2.Audits2StatusView.LoadingMessages=[{deviceType:'mobile',throttling:'on',message:'Lighthouse is loading your page with throttling to measure performance on a mobile device on 3G.',},{deviceType:'desktop',throttling:'on',message:'Lighthouse is loading your page with throttling to measure performance on a slow desktop on 3G.',},{deviceType:'mobile',throttling:'off',message:'Lighthouse is loading your page with mobile emulation.',},{deviceType:'desktop',throttling:'off',message:'Lighthouse is loading your page.',},];Audits2.Audits2StatusView.FastFacts=['1MB takes a minimum of 5 seconds to download on a typical 3G connection [Source: WebPageTest and DevTools 3G definition].','Rebuilding Pinterest pages for performance increased conversion rates by 15% [Source: WPO Stats]','BBC has seen a loss of 10% of their users for every extra second of page load [Source: WPO Stats]','By reducing the response size of JSON needed for displaying comments, Instagram saw increased impressions [Source: WPO Stats]','Walmart saw a 1% increase in revenue for every 100ms improvement in page load [Source: WPO Stats]','If a site takes >1 second to become interactive, users lose attention, and their perception of completing the page task is broken [Source: Google Developers Blog]','75% of global mobile users in 2016 were on 2G or 3G [Source: GSMA Mobile]','The average user device costs less than 200 USD. [Source: International Data Corporation]','53% of all site visits are abandoned if page load takes more than 3 seconds [Source: Google DoubleClick blog]','19 seconds is the average time a mobile web page takes to load on a 3G connection [Source: Google DoubleClick blog]','14 seconds is the average time a mobile web page takes to load on a 4G connection [Source: Google DoubleClick blog]','70% of mobile pages take nearly 7 seconds for the visual content above the fold to display on the screen. [Source: Think with Google]','As page load time increases from one second to seven seconds, the probability of a mobile site visitor bouncing increases 113%. [Source: Think with Google]','As the number of elements on a page increases from 400 to 6,000, the probability of conversion drops 95%. [Source: Think with Google]','70% of mobile pages weigh over 1MB, 36% over 2MB, and 12% over 4MB. [Source: Think with Google]','Lighthouse only simulates mobile performance; to measure performance on a real device, try WebPageTest.org [Source: Lighthouse team]',];Audits2.Audits2StatusView.fastFactRotationInterval=6000;Audits2.Audits2StatusView.minimumTextVisibilityDuration=3000;;Audits2.ProtocolService=class extends Common.Object{constructor(){super();this._rawConnection=null;this._backend=null;this._backendPromise=null;this._status=null;}
